...
# Embedded file name: compiler.pycodegen
...
class CodeGenerator():
    ...
    def _nameOp(self, prefix, name):
        ...
        if scope == SC_LOCAL:
            ...
            if scope == SC_GLOBAL_EXPLICIT:
                self.emit(prefix + '_GLOBAL', name)
            else:
                if scope == SC_GLOBAL_IMPLICIT:
                    if not self.optimized:
                        self.emit(prefix + '_NAME', name)
                    else:
                        self.emit(prefix + '_GLOBAL', name)
                else:
                    if scope == SC_FREE or scope == SC_CELL:
    ...
    def visitContinue(self, node):
        ...
        if kind == LOOP:
            ...
            if kind == EXCEPT or kind == TRY_FINALLY:
                self.set_lineno(node)
                top = len(self.setups)
                while top > 0:
                    top = top - 1
