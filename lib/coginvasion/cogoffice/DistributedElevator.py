...
# Embedded file name: lib.coginvasion.cogoffice.DistributedElevator
...
class DistributedElevator(DistributedObject):
    ...
    def fillSlot(self, index, avId):
        ...
        if toon:
            ...
            track.append(LerpPosInterval(toon, duration=0.5, pos=point, startPos=toon.getPos(self.getElevatorModel())))
            track.append(LerpHprInterval(toon, duration=0.1, hpr=(180, 0, 0), startHpr=toon.getHpr(self.getElevatorModel())))
    ...
    def emptySlot(self, index, avId):
        ...
        if toon:
            ...
            track = Sequence(Func(toon.animFSM.request, 'run'), LerpPosInterval(toon, duration=0.5, pos=OutPoint, startPos=InPoint), Func(toon.animFSM.request, 'neutral'), Func(toon.startSmooth))
            if avId == base.localAvatar.doId:
                self.localAvOnElevator = False
    ...
    def showHopOffButton(self):
        ...
        self.hopOffBtn = DirectButton(relief=None, text='Hop off', text_fg=(0.9, 0.9,
                                                                            0.9,
                                                                            1), text_pos=(0,
                                                                                          -0.23), text_scale=0.75, image=(upButton, downButton, rlvrButton), image_color=(0.5,
                                                                                                                                                                          0.5,
                                                                                                                                                                          0.5,
                                                                                                                                                                          1), image_scale=(20,
                                                                                                                                                                                           1,
                                                                                                                                                                                           11), pos=(0,
                                                                                                                                                                                                     0,
                                                                                                                                                                                                     0.8), scale=0.15, command=self.handleHopOffButton)
        return
