...
import DistributedToonInterior, CinemaGlobals
...
class DistributedCinemaInterior(DistributedToonInterior.DistributedToonInterior):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM('DCinemaInterior', [State.State('off', self.enterOff, self.exitOff),
         ...
         State.State('intermission', self.enterIntermission, self.exitIntermission)], 'off', 'off')
    ...
    def darkenInterior(self):
        darkenIval = self.interior.colorScaleInterval(3.0, colorScale=(0.3, 0.3, 0.3,
    ...
    def lightenInterior(self):
        lightenIval = self.interior.colorScaleInterval(3.0, colorScale=(1, 1, 1, 1.0), startColorScale=(0.3,
                                                                                                        0.3,
                                                                                                        0.3,
                                                                                                        1.0), blendType='easeInOut')
        lightenIval.start()
    ...
    def enterIntermission(self, ts=0):
        self.intermissionText = DirectLabel(relief=None, text_decal=True, text='', scale=0.7, parent=self.interior.find('**/sign_origin;+s'), text_font=CIGlobals.getMickeyFont(), text_fg=(1,
                                                                                                                                                                                            0.9,
                                                                                                                                                                                            0,
                                                                                                                                                                                            1))
