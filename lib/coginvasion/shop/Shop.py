...
class Shop(StateData):
    ...
    def __purchaseUpgradeItem(self, values):
        ...
        if upgrades < maxUpgrades and not hasTurret and turretCount < CogBattleGlobals.MAX_TURRETS:
            ...
            if turretCount >= CogBattleGlobals.MAX_TURRETS:
...
class ShopWindow(DirectFrame):
    ...
    def setup(self, title='CHOOSE WHAT YOU WANT TO BUY'):
        ...
        self.title = DirectLabel(text=title, relief=None, pos=(0, 0, 0.5), text_wordwrap=10, text_font=font, text_fg=(1,
                                                                                                                      1,
                                                                                                                      0,
                                                                                                                      1), scale=0.1, parent=self)
        self.updateTurretCount()
        self.infoLbl = DirectLabel(text='Welcome!', relief=None, text_scale=0.075, text_fg=txtFg, text_shadow=(0,
                                                                                                               0,
                                                                                                               0,
                                                                                                               0), pos=(0,
                                                                                                                        0,
                                                                                                                        0.215))
        self.okBtn = DirectButton(geom=CIGlobals.getOkayBtnGeom(), relief=None, text='OK', text_fg=txtFg, text_scale=txtScale, text_pos=txtPos, pos=(-0.1,
                                                                                                                                                     0,
                                                                                                                                                     -0.5), parent=self)
        self.clBtn = DirectButton(geom=CIGlobals.getCancelBtnGeom(), relief=None, text='Cancel', text_fg=txtFg, text_scale=txtScale, text_pos=txtPos, pos=(0.1,
                                                                                                                                                           0,
                                                                                                                                                           -0.5), parent=self)
        buttonGeom = (buttons.find('**/QT_back'), buttons.find('**/QT_back'), buttons.find('**/QT_back'), buttons.find('**/QT_back'))
        self.backBtn = DirectButton(geom=buttonGeom, relief=None, scale=0.05, pos=(-0.3,
                                                                                   0,
                                                                                   -0.25), parent=self, command=self.changePage, extraArgs=[0])
        self.nextBtn = DirectButton(geom=buttonGeom, relief=None, scale=0.05, pos=(0.3,
                                                                                   0,
                                                                                   -0.25), hpr=(0,
                                                                                                0,
                                                                                                180), command=self.changePage, extraArgs=[1], parent=self)
        self.hideInfo()
        return
    ...
    def updateTurretCount(self):
        ...
        if self.wantTurretCount:
            ...
            if not self.turretImg:
                self.turretImg = OnscreenImage(image='phase_3.5/maps/cannon-icon.png', scale=(0.05,
                                                                                              1,
                                                                                              0.05), pos=(-0.22,
                                                                                                          0,
                                                                                                          0.275))
                self.turretImg.setTransparency(TransparencyAttrib.MAlpha)
            self.turretLabel = DirectLabel(text='Turrets: %s/%s' % (str(self.turretCount), str(maxTurrets)), relief=None, text_scale=0.07, text_fg=(0,
                                                                                                                                                    0,
                                                                                                                                                    0,
                                                                                                                                                    1), text_shadow=(0,
                                                                                                                                                                     0,
    ...
    def __makeGagEntry(self, pos, item, values, page):
        ...
        button = DirectButton(geom=itemImage, scale=1.3, pos=pos, relief=None, parent=page, command=self.shop.purchaseItem, extraArgs=[item, page])
        ...
        buttonLabel = DirectLabel(text='%s/%s\n%s JBS' % (str(supply), str(maxSupply),
         str(values.get('price'))), relief=None, parent=button, text_scale=0.05, pos=(0,
                                                                                      0,
                                                                                      -0.11))
    ...
    def __makeUpgradeEntry(self, pos, item, values, page):
        itemImage = values.get('image')
        button = DirectButton(image=itemImage, scale=0.15, pos=pos, relief=None, parent=page, command=self.shop.purchaseItem, extraArgs=[
         item, page])
        ...
        buttonLabel = DirectLabel(text='%s\n%s/%s\n%s JBS' % (item, str(supply), str(maxSupply),
         str(values.get('price'))), relief=None, parent=button, text_scale=0.3, pos=(0,
                                                                                     0,
                                                                                     -1.2))
        self.addEntryToPage(page, item, values, button, buttonLabel)
        return
    ...
    def __makeHealEntry(self, pos, item, values, page):
        ...
        button = DirectButton(image=itemImage, scale=0.105, pos=pos, relief=None, parent=page, command=self.shop.purchaseItem, extraArgs=[item, page])
        button.setTransparency(TransparencyAttrib.MAlpha)
        buttonLabel = DirectLabel(text=label, relief=None, parent=button, text_scale=0.55, pos=(0,
                                                                                                0,
                                                                                                -1.6))
        self.addEntryToPage(page, item, values, button, buttonLabel)
        return
    ...
    def makePages(self, items):
        ...
        if self.nPages % 4 != 0 and len(newItems) > 4:
            ...
            if self.nPages == 0:
                self.nPages = 1
        itemPos = [
         (-0.45, 0, 0), (-0.15, 0, 0), (0.15, 0, 0), (0.45, 0, 0)]
        pageIndex = 0
        itemIndex = 0
        index = 0
    ...
    def delete(self):
        elements = [
         self.title, self.okBtn, self.clBtn, self.infoLbl, self.backBtn, self.nextBtn, self.turretLabel, self.turretImg]
