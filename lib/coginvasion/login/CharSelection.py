...
class CharSelection(DirectObject):
    ...
    def __init__(self, avChooser):
        ...
        self.selectionFSM = ClassicFSM.ClassicFSM('CharSelection', [
         ...
         State.State('empty', self.enterEmptySelected, self.exitEmptySelected)], 'off', 'off')
    ...
    def deleteToon(self, avId):
        self.deleteConf = Dialog.GlobalDialog(message=('This will delete {0} forever. Are you sure?').format(self.avChooser.getNameFromAvId(avId)), style=Dialog.YesNo, doneEvent='deleteConfResponse', extraArgs=[avId])
    ...
    def load(self):
        ...
        for door in doors:
            for frameHole in door.findAllMatches('**/doorFrameHole*'):
                frameHole.removeNode()

        if holidayMgr.getHoliday() == HolidayType.CHRISTMAS:
            piles = {'half': {'pos': (57.28, 86.47, -25.0), 'hpr': (46.79, 0, 0)}, 'full': {'pos': (71.23, 85.2, -25.0), 'hpr': (290.82, 0, 0)}, 'half_2': {'pos': (-15, 128.69, -25), 'hpr': (60.26, 0, 0)}}
            ...
            base.render.setFog(self.fog)
        self.title = DirectLabel(text=self.TITLE, text_font=CIGlobals.getMickeyFont(), text_fg=(1,
                                                                                                0.9,
                                                                                                0.1,
                                                                                                1), relief=None, text_scale=0.13, pos=(0,
                                                                                                                                       0,
                                                                                                                                       0.82))
        self.charNameLabel = OnscreenText(text='', font=CIGlobals.getMickeyFont(), pos=(-0.25,
                                                                                        0.5,
                                                                                        0), fg=(1,
                                                                                                0.9,
                                                                                                0.1,
        ...
        self.playOrCreateButton = DirectButton(text='', pos=(0.8125, 0, -0.35), command=self.__action, geom=CIGlobals.getDefaultBtnGeom(), text_scale=0.06, relief=None, text_pos=(0,
                                                                                                                                                                                   -0.01))
        self.playOrCreateButton.hide()
        self.deleteButton = DirectButton(text='Delete', pos=(0.8125, 0, -0.45), command=self.__action, extraArgs=['delete'], geom=CIGlobals.getDefaultBtnGeom(), text_scale=0.06, relief=None, text_pos=(0,
                                                                                                                                                                                                         -0.01))
        self.deleteButton.hide()
        self.quitButton = DirectButton(text='Quit', pos=(-1.1, 0, -0.925), command=self.__action, extraArgs=[
         'quit'], text_scale=0.06, geom=CIGlobals.getDefaultBtnGeom(), relief=None, text_pos=(0,
                                                                                              -0.01))
        ...
        for slot in range(6):
            if self.avChooser.hasToonInSlot(slot):
                ...
                text = self.NO_TOON
            btn = DirectButton(relief=None, text=text, text_scale=0.06, text_align=TextNode.ALeft, text1_bg=textDownColor, text2_bg=textRolloverColor, text3_fg=textDisabledColor, textMayChange=0, command=self.__handleCharButton, extraArgs=[
             slot], text_pos=(0, 0, 0.0))
        ...
        self.charList = DirectScrolledList(relief=None, pos=(0.75, 0, -0.225), incButton_image=None, incButton_scale=(
         arrowButtonScale, arrowButtonScale, -arrowButtonScale), decButton_image=None, decButton_scale=(
         arrowButtonScale, arrowButtonScale, arrowButtonScale), itemFrame_pos=(
         ...
         listZorigin + listFrameSizeZ), itemFrame_frameColor=(0.85, 0.95, 1, 1), itemFrame_borderWidth=(0.01,
                                                                                                        0.01), numItemsVisible=15, forceHeight=0.075, items=self.charButtons, parent=self.frame)
