...
# Embedded file name: lib.coginvasion.gags.FireHose
...
class FireHose(SquirtGag):
    ...
    def __init__(self):
        SquirtGag.__init__(self, CIGlobals.FireHose, 'phase_5/models/props/firehose-mod.bam', 30, GagGlobals.NULL_SFX, GagGlobals.FIREHOSE_SFX, GagGlobals.NULL_SFX, 'firehose', 0, 0)
    ...
    def start(self):
        ...
        propTrack = Sequence(Func(self.hydrantNode.reparentTo, self.avatar), LerpScaleInterval(self.hydrantScale, tAppearDelay * 0.5, Point3(1, 1, 1.4), startScale=Point3(1, 1, 0.01)), LerpScaleInterval(self.hydrantScale, tAppearDelay * 0.3, Point3(1, 1, 0.8), startScale=Point3(1, 1, 1.4)), LerpScaleInterval(self.hydrantScale, tAppearDelay * 0.1, Point3(1, 1, 1.2), startScale=Point3(1, 1, 0.8)), LerpScaleInterval(self.hydrantScale, tAppearDelay * 0.1, Point3(1, 1, 1), startScale=Point3(1, 1, 1.2)), ActorInterval(self.gag, 'chan', duration=dAnimHold), Wait(dHoseHold - 0.2), LerpScaleInterval(self.hydrantScale, 0.2, Point3(1, 1, 0.01), startScale=Point3(1, 1, 1)), Func(cleanupHydrantNode))
