...
# Embedded file name: lib.coginvasion.minigame.DistributedGunGameCapturePoint
...
class DistributedGunGameCapturePoint(DistributedNode):
    ...
    def getCircleScaleAnim(self, startScale, scale, duration=1.5):
        return Sequence(LerpScaleInterval(self.captureCircle, startScale=startScale, scale=scale, duration=duration), Func(self.captureCircle.setColorScale, 0.976, 0, 0, 0.5), Wait(0.25), Func(self.captureCircle.setColorScale, 1, 1, 1, 0.5))
    ...
    def startCircleAnim(self, direction, timestamp):
        ...
        if direction == 0:
            self.circleTrack = Sequence(Func(self.captureCircle.show), Func(self.captureCircle.setColorScale, 1, 1, 1, 0.5), LerpScaleInterval(self.captureCircle, startScale=0.75, scale=3.75, duration=7.5), Func(self.captureCircle.setColorScale, 0.976, 0, 0, 0.5), Wait(0.25), Func(self.captureCircle.setColorScale, 1, 1, 1, 0.5), Wait(0.25), Func(self.captureCircle.hide))
            self.circleTrack.start(timestamp)
            return
        if direction == 1:
            self.circleTrack = Sequence(Func(self.captureCircle.show), Func(self.captureCircle.setColorScale, 0.976, 0, 0, 0.5), Wait(0.25), Func(self.captureCircle.setColorScale, 1, 1, 1, 0.5))
    ...
    def handleContesters(self, contesters):
        if self.circleTrack:
            ...
            elif contesters > 0 and not self.team:
                self.circleTrack.pause()
            else:
                self.circleTrack.resume()
