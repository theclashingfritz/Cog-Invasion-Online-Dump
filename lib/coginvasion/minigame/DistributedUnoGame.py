...
import DistributedMinigame, UnoGameGlobals as UGG, UnoGameCardDeck
...
class DistributedUnoGame(DistributedMinigame.DistributedMinigame):
    ...
    def createGui(self):
        ...
        self.drawBtn = DirectButton(text='Draw', relief=None, text_scale=0.055, scale=1, parent=base.a2dBottomLeft, geom=(
         gui.find('**/QuitBtn_UP'),
         gui.find('**/QuitBtn_DN'),
        ...
        self.callBtn = DirectButton(geom=(uno_gui.find('**/mg_uno_call_btn-idle'),
         ...
         uno_gui.find('**/mg_uno_call_btn-disabled')), relief=None, scale=0.3, parent=base.a2dBottomLeft, pos=(0.18,
                                                                                                               0,
                                                                                                               0.305), command=self.d_callUno)
        self.callBtn['state'] = DGG.DISABLED
        self.playByPlay = OnscreenText(text='', font=CIGlobals.getMickeyFont(), scale=0.1, fg=(0,
                                                                                               0.8,
                                                                                               0,
                                                                                               1), pos=(0,
                                                                                                        0.85), wordwrap=15.0)
    ...
    def setPlayerTurn(self, doId):
        if self.numCards == 1:
            ...
            if self.numCards == 0 and not self.hasCalledZero:
    ...
    def createNewColorGui(self):
        ...
        self.newColorDialog = OnscreenImage(image=DGG.getDefaultDialogGeom(), scale=(1.5,
                                                                                     1,
                                                                                     1))
        self.newColorTitle = DirectLabel(text='Choose A Card Color', text_scale=0.12, text_pos=(0,
                                                                                                0.3), relief=None)
        self.newColorBlue = DirectButton(image=gui.find('**/mg_uno_numcards_blue_blank'), relief=None, scale=(0.2,
                                                                                                              0.35,
                                                                                                              0.35), pos=(-0.325,
                                                                                                                          0,
                                                                                                                          -0.075), command=self.d_takeNewCardColor, extraArgs=[
         str(UGG.CARD_BLUE)])
        self.newColorRed = DirectButton(image=gui.find('**/mg_uno_numcards_red_blank'), relief=None, scale=(0.2,
                                                                                                            0.35,
                                                                                                            0.35), pos=(-0.125,
                                                                                                                        0,
                                                                                                                        -0.075), command=self.d_takeNewCardColor, extraArgs=[
         str(UGG.CARD_RED)])
        self.newColorGreen = DirectButton(image=gui.find('**/mg_uno_numcards_green_blank'), relief=None, scale=(0.2,
                                                                                                                0.35,
                                                                                                                0.35), pos=(0.075,
                                                                                                                            0,
                                                                                                                            -0.075), command=self.d_takeNewCardColor, extraArgs=[
         str(UGG.CARD_GREEN)])
        self.newColorYellow = DirectButton(image=gui.find('**/mg_uno_numcards_yellow_blank'), relief=None, scale=(0.2,
                                                                                                                  0.35,
                                                                                                                  0.35), pos=(0.275,
                                                                                                                              0,
                                                                                                                              -0.075), command=self.d_takeNewCardColor, extraArgs=[
    ...
    def placeCard(self, doId, cardId):
        ...
        if self.localAvId != doId:
            LerpPosInterval(card, duration=1.0, pos=(0, 0, 0.1), startPos=(-1.5, 0,
                                                                           1.5), blendType='easeOut').start()
        else:
            self.numCards -= 1
            LerpPosInterval(card, duration=1.0, pos=(0, 0, 0.1), startPos=(0, 0, -1.5), blendType='easeOut').start()
            LerpScaleInterval(card, duration=1.0, scale=(0.4, 0.49, 0.49), startScale=(0.3,
                                                                                       0.3,
                                                                                       0.23), blendType='easeOut').start()
