...
class RemoteToonBattleAvatar(RemoteAvatar):
    ...
    def enterShoot(self, ts):
        if self.avatar:
            ...
            self.track = Sequence(Func(createBullet), ActorInterval(self.avatar, 'squirt', partName='torso', startFrame=48, endFrame=58), ActorInterval(self.avatar, 'squirt', partName='torso', startFrame=107, endFrame=126, playRate=3), Func(changeToLegAnim))
            self.track.start()
            del gunSound
