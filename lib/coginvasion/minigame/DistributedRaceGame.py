...
import RaceGameMovement, DistributedMinigame
...
class DistributedRaceGame(DistributedMinigame.DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        self.raceFSM = ClassicFSM('DistributedRaceGame', [State('race', self.enterRace, self.exitRace),
         ...
         State('off', self.enterRaceOff, self.exitRaceOff)], 'off', 'off')
        ...
        self.lanePos = [(-22.0, -205.0, 0.0),
         ...
         (0.0, -205.0, 0.0),
         (-33.66, -205.0, 0.0)]
        self.initialCamPos = {'pos': (41.1, -145.0, 25.88), 'hpr': (135.0, 345.96, 0.0)}
    ...
    def startCountdown(self):
        self.countdownLbl = DirectLabel(text='', text_scale=0.3, text_font=CIGlobals.getMickeyFont(), text_fg=(1,
                                                                                                               1,
                                                                                                               0,
                                                                                                               1), pos=(0,
                                                                                                                        0,
    ...
    def moveCameraToToon(self):
        camPInt = LerpPosInterval(camera, duration=3.0, pos=self.localAv.getPos(render) + (0,
                                                                                           15,
                                                                                           3), startPos=camera.getPos(render), blendType='easeInOut')
