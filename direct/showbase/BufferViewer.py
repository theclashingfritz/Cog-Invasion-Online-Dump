...
class BufferViewer(DirectObject):
    ...
    def analyzeTextureSet(self, x, set):
        if isinstance(x, list):
            ...
            if isinstance(x, Texture):
                set[x] = 1
            else:
                if isinstance(x, GraphicsOutput):
                    for itex in range(x.countTextures()):
                        tex = x.getTexture(itex)
                        set[tex] = 1

                else:
                    if isinstance(x, GraphicsEngine):
    ...
    def maintainReadout(self, task):
        ...
        if self.layout == 'hline':
            ...
            if self.layout == 'vline':
                rows = ncards
                cols = 1
            else:
                if self.layout == 'hgrid':
        ...
        if self.position == 'llcorner':
            ...
            if self.position == 'lrcorner':
                dirx = 1.0
                diry = -1.0
            else:
                if self.position == 'ulcorner':
                    dirx = -1.0
                    diry = 1.0
                else:
                    if self.position == 'urcorner':
                        dirx = 1.0
                        diry = 1.0
                    else:
