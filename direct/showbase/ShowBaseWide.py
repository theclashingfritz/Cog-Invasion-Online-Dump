...
# Embedded file name: direct.showbase.ShowBaseWide
__all__ = [
...
class ShowBase(DirectObject.DirectObject):
    ...
    def __init__(self, fStartDirect=True, windowType=None):
        ...
        logStackDump = self.config.GetBool('log-stack-dump', False) or self.config.GetBool('client-log-stack-dump', False)
    ...
    def makeCamera(self, win, sort=0, scene=None, displayRegion=(
 0, 1, 0, 1), stereo=None, aspectRatio=None, clearDepth=0, clearColor=None, lens=None, camName='cam', mask=None, useCamera=None):
        if self.camera == None:
    ...
    def makeCamera2d(self, win, sort=10, displayRegion=(
 0, 1, 0, 1), coords=(-1, 1, -1, 1), lens=None, cameraName=None):
        dr = win.makeMonoDisplayRegion(*displayRegion)
        dr.setSort(sort)
        dr.setClearDepthActive(1)
    ...
    def makeCamera2dp(self, win, sort=20, displayRegion=(
 0, 1, 0, 1), coords=(-1, 1, -1, 1), lens=None, cameraName=None):
        dr = win.makeMonoDisplayRegion(*displayRegion)
        dr.setSort(sort)
    ...
    def windowEvent(self, win):
        ...
        if properties != self.__prevWindowProperties:
            ...
            if properties.getForeground() and not self.mainWinForeground:
                ...
                if not properties.getForeground() and self.mainWinForeground:
                    self.mainWinForeground = 0
                    if self.__autoGarbageLogging:
                        GarbageReport.b_checkForGarbageLeaks()
            if properties.getMinimized() and not self.mainWinMinimized:
                ...
                if not properties.getMinimized() and self.mainWinMinimized:
                    self.mainWinMinimized = 0
