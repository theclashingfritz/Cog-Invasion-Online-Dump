...
class Messenger:
    ...
    def __init__(self):
        ...
        self.lock = Lock()
        self.quieting = {'NewFrame': 1, 'avatarMoving': 1, 
           'event-loop-done': 1, 
        ...
        try:
            ...
            if id in acceptorDict:
                if notifyDebug:
                    ...
                    if oldMethod == method:
                        self.notify.warning('object: %s was already accepting: "%s" with same callback: %s()' % (
                         object.__class__.__name__, safeRepr(event), method.__name__))
                    else:
                        self.notify.warning('object: %s accept: "%s" new callback: %s() supplanting old callback: %s()' % (
                         object.__class__.__name__, safeRepr(event), method.__name__, oldMethod.__name__))
            acceptorDict[id] = [
             method, extraArgs, persistent]
    ...
    def send(self, event, sentArgs=[], taskChain=None):
        ...
        try:
            ...
            if taskChain:
                ...
                if len(queue) == 1:
                    taskMgr.add(self.__taskChainDispatch, name='Messenger-%s' % taskChain, extraArgs=[
        ...
        for entry in self.__callbacks.items():
            ...
            for objectEntry in objectDict.items():
                ...
                if function == oldMethod:
                    newMethod = types.MethodType(newFunction, method.im_self, method.im_class)
                    params[0] = newMethod
                    retFlag += 1
