...
class DoInterestManager(DirectObject.DirectObject):
    ...
    def addInterest(self, parentId, zoneIdList, description, event=None):
        ...
        if parentId not in (self.getGameDoId(),):
            ...
            if not parent:
                ...
                DoInterestManager.notify.error('addInterest: no setParentingRules defined in the DC for object %s (%s)' % (
    ...
    def _sendRemoveAIInterest(self, handle):
        datagram = PyDatagram()
        datagram.addUint16(CLIENT_REMOVE_INTEREST)
        datagram.addUint16(32768 + handle)
        self.send(datagram)
    ...
    def handleInterestDoneMessage(self, di):
        ...
        DoInterestManager.notify.debug('handleInterestDoneMessage--> Received handle %s, context %s' % (
         handle, contextId))
        if handle in DoInterestManager._interests:
            eventsToSend = []
            if contextId == DoInterestManager._interests[handle].context:
                ...
                DoInterestManager.notify.debug('handleInterestDoneMessage--> handle: %s: Expecting context %s, got %s' % (
