...
class ClockDelta(DirectObject.DirectObject):
    ...
    def peerToPeerResync(self, avId, timestamp, serverTime, uncertainty):
        ...
        if elapsed <= 0 or elapsed > P2PResyncDelay:
            ...
            self.notify.info('Got sync +/- %.3f s, elapsed %.3f s, from %s.' % (
    ...
    def newDelta(self, localTime, newDelta, newUncertainty, trustNew=1):
        ...
        if oldUncertainty != None:
            self.notify.info('previous delta at %.3f s, +/- %.3f s.' % (
             self.delta, oldUncertainty))
            self.notify.info('new delta at %.3f s, +/- %.3f s.' % (
            ...
            if low > high:
                ...
                self.notify.info('intersection at %.3f s, +/- %.3f s.' % (
                 newDelta, newUncertainty))
        self.delta = newDelta
